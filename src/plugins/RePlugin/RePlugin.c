/* Smalltalk from Squeak4.6 with VMMaker 4.19.13 translated as C source on 17 January 2022 6:30:19 pm */
/* Automatically generated by
	SmartSyntaxPluginCodeGenerator VMMaker-dtl.432 uuid: 89fa5bee-431a-474e-96da-dd7640dd7227
   from
	RePlugin VMMaker-dtl.432 uuid: 89fa5bee-431a-474e-96da-dd7640dd7227
 */
static char __buildInfo[] = "RePlugin VMMaker-dtl.432 uuid: 89fa5bee-431a-474e-96da-dd7640dd7227 " __DATE__ ;
...
/*** Function Prototypes ***/
static sqInt allocateByteArrayAndSetRcvrExtraPtrFrom(pcre_extra *anExtraPtr);
static sqInt allocateByteArrayAndSetRcvrPCREPtrFromPCRE(pcre *aPCREPtr);
...
#ifdef SQUEAK_BUILTIN_PLUGIN
	"RePlugin 17 January 2022 (i)"
#else
	"RePlugin 17 January 2022 (e)"
#endif
;
...
static sqInt allocateByteArrayAndSetRcvrExtraPtrFrom(pcre_extra *anExtraPtr) {
	...
	if (anExtraPtr) {
		...
		memcpy(extraByteArrayPtr, anExtraPtr, sizeof(real_pcre_extra));
	} else {
...
static sqInt allocateByteArrayAndSetRcvrPCREPtrFromPCRE(pcre *aPCREPtr) {
	...
	memcpy(patByteArrayPtr, aPCREPtr, lastAlloc);
...
EXPORT(sqInt) primPCRECompile(void) {
	...
	pcrePtr = pcre_compile(patternStrPtr, compileFlags, &errorStrBuffer, &errorOffset, NULL);
	if (pcrePtr) {
		allocateByteArrayAndSetRcvrPCREPtrFromPCRE((pcre  *)pcrePtr);
		extraPtr = pcre_study(pcrePtr, compileFlags, &errorStrBuffer);
		allocateByteArrayAndSetRcvrExtraPtrFrom((pcre_extra  *)extraPtr);
		/* begin rePluginFree: */
		numFrees += 1;
		if (pcrePtr != null) {
			free(pcrePtr);
...
EXPORT(sqInt) primPCREExec(void) {
	...
	pcrePtr = ((pcre *) (interpreterProxy->fetchArrayofObject(2, rcvr)));
	/* begin rcvrExtraPtr */
	extraObj = interpreterProxy->fetchPointerofObject(3, rcvr);
	extraPtr = (pcre_extra *)((pcre_extra *) ((extraObj == (interpreterProxy->nilObject())
	? null
	...
	result = pcre_exec(pcrePtr, extraPtr, 
...
EXPORT(sqInt) primPCRENumSubPatterns(void) {
	...
	rcvr = interpreterProxy->stackObjectValue(0);
	pcrePtr = ((pcre *) (interpreterProxy->fetchArrayofObject(2, rcvr)));
	pcre_fullinfo(pcrePtr, NULL, PCRE_INFO_CAPTURECOUNT, &ncap);
	interpreterProxy->pop(1);
