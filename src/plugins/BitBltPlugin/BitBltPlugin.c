/* Smalltalk from Squeak4.6 with VMMaker 4.20.3 translated as C source on 30 December 2022 5:14:15 pm */
/* Automatically generated by
	SmartSyntaxPluginCodeGenerator VMMaker-dtl.437 uuid: 720c8b4b-1edc-4154-8f0e-dfb30db137a7
   from
	BitBltSimulation VMMaker-dtl.437 uuid: 720c8b4b-1edc-4154-8f0e-dfb30db137a7
 */
static char __buildInfo[] = "BitBltSimulation VMMaker-dtl.437 uuid: 720c8b4b-1edc-4154-8f0e-dfb30db137a7 " __DATE__ ;
...
static sqInt dx;
static sqInt dy;
static sqInt endOfDestination;
static sqInt endOfSource;
...
#ifdef SQUEAK_BUILTIN_PLUGIN
	"BitBltPlugin 30 December 2022 (i)"
#else
	"BitBltPlugin 30 December 2022 (e)"
#endif
;
static sqInt nWords;
...
static unsigned int alphaBlendUnscaledwith(unsigned int sourceWord, unsigned int destinationWord) {
	...
	blendR = (((((sourceWord & 0xFF0000) * alpha) + ((destinationWord & 0xFF0000) * (blendA - alpha))) + (blendA << 15)) / blendA) & 0xFF0000;

	/* blend green */

	blendG = (((((sourceWord & 0xFF00) * alpha) + ((destinationWord & 0xFF00) * (blendA - alpha))) + (blendA << 7)) / blendA) & 0xFF00;

	/* blend blue */

	blendB = (((((sourceWord & 0xFF) * alpha) + ((destinationWord & 0xFF) * (blendA - alpha))) + (((usqInt) blendA) >> 1)) / blendA) & 0xFF;
	result = ((blendR | blendB) | blendG) | (blendA << 24);
	return result;
}
...
static unsigned int alphaScalewith(unsigned int sourceWord, unsigned int destinationWord) {
	...
	rb = ((destinationWord & 16711935) * alpha) + 8388736;

	/* divide by 255 */

	rb = (((usqInt) (rb + ((((usqInt) (rb - 65537)) >> 8) & 16711935))) >> 8) & 16711935;

	/* scale green component */

	g = ((destinationWord & 0xFF00) * alpha) + 32768;

	/* divide by 255 */

	g = (((usqInt) (g + ((((usqInt) (g - 256)) >> 8) & 0xFF00))) >> 8) & 0xFF00;
...
static unsigned int alphaUnscalewith(unsigned int sourceWord, unsigned int destinationWord) {
	...
	r = (((destinationWord & 0xFF0000) * 0xFF) + ((alpha + 1) << 15)) / alpha;

	/* unscale green component */

	g = (((destinationWord & 0xFF00) * 0xFF) + ((alpha + 1) << 7)) / alpha;
	...
	carry = carry | ((((usqInt) (carry & 11184810)) >> 1) | ((carry & 5592405) << 1));
	carry = carry | ((((usqInt) (carry & 13421772)) >> 2) | ((carry & 3355443) << 2));
	carry = carry | ((((usqInt) (carry & 15790320)) >> 4) | ((carry & 986895) << 4));
