/* Smalltalk from Squeak4.6 with VMMaker 4.20.2 translated as C source on 20 December 2022 3:41:44 pm */
/* Automatically generated by
	VMPluginCodeGenerator VMMaker-dtl.436 uuid: fce55fb3-ce12-4897-a865-75341a26701e
   from
	UnixOSProcessPlugin VMConstruction-Plugins-OSProcessPlugin-dtl.56 uuid: 459fcf26-c73a-4bc2-96f9-c6cfb7433887
 */
static char __buildInfo[] = "UnixOSProcessPlugin VMConstruction-Plugins-OSProcessPlugin-dtl.56 uuid: 459fcf26-c73a-4bc2-96f9-c6cfb7433887 " __DATE__ ;
...
/* D T Lewis - UnixOSProcessPlugin.c translated from class
   UnixOSProcessPlugin of OSProcessPlugin version 4.6.8 */
#include <sys/time.h>
#include <sys/wait.h>
#include <signal.h>
#include <unistd.h>
...
static sqInt execargVecBufferargVecOffsetsenvVecBufferenvVecOffsets(sqInt executableFile, sqInt argVecBuffer, sqInt argOffsets, sqInt envVecBuffer, sqInt envOffsets);
...
#pragma export on
EXPORT(char **) listOfCStringsInwithOffsets(sqInt buffer, sqInt stringOffsets);
#pragma export off
...
static sqInt prepareForExecstdInstdOutstdErr(sqInt workingDir, sqInt stdIn, sqInt stdOut, sqInt stdErr);
#pragma export on
...
#ifdef SQUEAK_BUILTIN_PLUGIN
	"UnixOSProcessPlugin 20 December 2022 (i)"
#else
	"UnixOSProcessPlugin 20 December 2022 (e)"
#endif
;
...
/*	Answer a new collection, usually of type ByteArray or ByteString copied from
	a null-terminated C string. Caution: This may invoke the garbage collector. */
...
static sqInt cStringasCollection(const char *aCString, sqInt classIdentifier) {
	...
	len = strlen(aCString);
	newString = interpreterProxy->instantiateClassindexableSize(classIdentifier, len);
...
/*	Call execve to overlay the current process with a new executable program. */
/*	Set up the environment, a C array of pointers to C strings, where each string
	is of the form 'KEY=value'. Use space in the object memory to avoid malloc().: */
...
static sqInt execargVecBufferargVecOffsetsenvVecBufferenvVecOffsets(sqInt executableFile, sqInt argVecBuffer, sqInt argOffsets, sqInt envVecBuffer, sqInt envOffsets) {
	...
	if (envVecBuffer == (interpreterProxy->nilObject())) {
		...
		env = listOfCStringsInwithOffsets(envVecBuffer, envOffsets);
	}
	args = listOfCStringsInwithOffsets(argVecBuffer, argOffsets);

	/* Overlay a new executable program onto this process address space. */

	progName = interpreterProxy->arrayValueOf(executableFile);
	if ((execve(progName, args, env)) == -1) {
		perror(progName);
		_exit(-1);
	} else {
...
# if (SA_ONSTACK)
	...
	/* true if platform supports sigaltstack, else meaningless redeclarations of stack_t and sigaction */
...
/*	Initialize a buffer with the addresses of strings within the buffer. The buffer contains
	null terminated C strings, and stringOffsets are the relative position of those strings
	within the buffer. Answer a pointer to a list of C strings. */
/*	for debugging */
/*	for debugging */
...
EXPORT(char **) listOfCStringsInwithOffsets(sqInt buffer, sqInt stringOffsets) {
    char *bufferAddress;
    sqInt count;
    sqInt *stringAddresses;

	count = interpreterProxy->stSizeOf(stringOffsets);
	bufferAddress = interpreterProxy->arrayValueOf(buffer);
	stringAddresses = interpreterProxy->firstIndexableField(stringOffsets);
	fixPointersInArrayOfStringswithOffsetscount(bufferAddress, stringAddresses, count);
	return ((char **) bufferAddress);
}
...
/*	Assume we are running in a new child process after a unix process fork,.
	Prepare to exec a new program in the new process. */
...
EXPORT(sqInt) primitiveArgumentAt(void) {
	...
	classIdentifier = interpreterProxy->classString();
	...
	if ((index > count) || (index < 1)) {
		...
		sPtr = args[index - 1];
		s = cStringasCollection(sPtr, classIdentifier);
		interpreterProxy->pop(2);
...
/*	Answer a byte array containing the OS process argument at index (an Integer)
...
EXPORT(sqInt) primitiveArgumentAtAsBytes(void) {
	...
	/* begin argumentAtAsType: */
	classIdentifier = interpreterProxy->classByteArray();
	index = interpreterProxy->stackIntegerValue(0);
	count = getArgumentCount();
	if ((index > count) || (index < 1)) {
		interpreterProxy->pop(2);
...
EXPORT(sqInt) primitiveConfstr(void) {
	...
	optionIndex = interpreterProxy->stackIntegerValue(0);
	...
	bufferString = interpreterProxy->instantiateClassindexableSize(interpreterProxy->classString(), bufferSize);
	buffer = interpreterProxy->firstIndexableField(bufferString);
	result = confstr(optionIndex, buffer, bufferSize);
	interpreterProxy->popthenPush(2, stringFromCString(buffer));
	return null;
}
...
EXPORT(sqInt) primitiveEnvironmentAtSymbol(void) {
	...
	if (getenvResult == 0) {
		...
		interpreterProxy->pop(2);
		interpreterProxy->push(cStringasCollection(getenvResult, classIdentifier));
	}
	return null;
}
...
EXPORT(sqInt) primitiveErrorMessageAt(void) {
	...
	errMessage = cStringasCollection(p, interpreterProxy->classString());
	interpreterProxy->pop(2);
...
/*	Fork a child OS process, and do an exec in the child. The parent continues on in
	Smalltalk, and this method answers the pid of the child that was created.

	Memory for the argument and environment arrays is allocated in the image prior
	to calling the primitive in order to avoid invoking the garbage collector in the primitive.
...
EXPORT(sqInt) primitiveForkExec(void) {
	...
	the child process will receive a timer interrupt, and will not know how to handle it. */
		...
		if (pid == 0) {
			...
			prepareForExecstdInstdOutstdErr(interpreterProxy->stackObjectValue(0), interpreterProxy->stackObjectValue(7), interpreterProxy->stackObjectValue(6), interpreterProxy->stackObjectValue(5));
			execargVecBufferargVecOffsetsenvVecBufferenvVecOffsets(interpreterProxy->stackObjectValue(8), interpreterProxy->stackObjectValue(4), interpreterProxy->stackObjectValue(3), interpreterProxy->stackObjectValue(2), interpreterProxy->stackObjectValue(1));
		} else {
...
EXPORT(sqInt) primitiveFpathconf(void) {
	...
	/* begin fileDescriptorFrom: */
	aSQFileByteArray = interpreterProxy->stackValue(1);
	if (!((((interpreterProxy->isBytes(aSQFileByteArray)) && ((interpreterProxy->byteSizeOf(aSQFileByteArray)) == (sizeof(SQFile)))) && ((interpreterProxy->getThisSessionID()) == (sessionIdentifierFromSqFile(interpreterProxy->arrayValueOf(aSQFileByteArray))))) && (isNonNullSQFile(aSQFileByteArray)))) {
		fd = -1;
		goto l1;
	}
...
l1:	/* end fileDescriptorFrom: */;
	...
	interpreterProxy->popthenPush(3, interpreterProxy->signed32BitIntegerFor(result));
	return null;
}
...
EXPORT(sqInt) primitiveGetCurrentWorkingDirectory(void) {
	...
	if (cwd == 0) {
		...
		cwdString = cStringasCollection(cwd, classIdentifier);
		interpreterProxy->pop(1);
...
EXPORT(sqInt) primitivePathconf(void) {
	...
	pathString = interpreterProxy->stackValue(1);
...
EXPORT(sqInt) primitiveRealpath(void) {
	...
	if (realpathResult == 0) {
		...
		s = cStringasCollection(realpathResult, classIdentifier);
		interpreterProxy->pop(2);
		interpreterProxy->push(s);
	}
	return null;
...
/*	Answer a byte array with the real path for a path string as determined by realpath(). */
...
EXPORT(sqInt) primitiveSpaceForByteSizeInDirectoryPath(void) {
	...
	dirPath = cStringFromString(interpreterProxy->stackValue(0));
	bytesRequired = interpreterProxy->stackIntegerValue(1);
	...
	interpreterProxy->popthenPush(3, fileSystemFull);
	return null;
}


/*	Answer the result of sysconf() for the supplied option index, an integer
	value defined in unistd.h or limits.h. On some platforms, the available index
	values are defined in /usr/include/bits/confname.h */
...
EXPORT(sqInt) primitiveSysconf(void) {
	...
	interpreterProxy->popthenPush(2, interpreterProxy->signed32BitIntegerFor(result));
	return null;
}
...
# if (SA_ONSTACK)
	...
	/* true if platform supports sigaltstack */
...
# if (SA_DISABLE)
	...
	/* Use existing signal stack if available. Mac OS documents SA_DISABLE but defines SS_DISABLE */
...
static sqInt sqFileFromforWriteisStdio(FILEHANDLETYPE filehandle, sqInt writeFlag, sqInt isInputOutputOrErrorStream) {
	...
	isStdioStream = isInputOutputOrErrorStream;
	thisSession = interpreterProxy->getThisSessionID();
	...
	sqFilePtr->lastOp = 0;
	sqFilePtr->isStdioStream = (char) isStdioStream;
	return sqFile;
}
...
static sqInt stringFromCString(const char *aCString) {
	return cStringasCollection(aCString, interpreterProxy->classString());
}
...
void* UnixOSProcessPlugin_exports[][3] = {
	...
	{"UnixOSProcessPlugin", "primitiveConfstr", (void*)primitiveConfstr},
	...
	{"UnixOSProcessPlugin", "primitiveArgumentAtAsBytes", (void*)primitiveArgumentAtAsBytes},
	...
	{"UnixOSProcessPlugin", "primitiveConnectToFileDescriptor", (void*)primitiveConnectToFileDescriptor},
	{"UnixOSProcessPlugin", "primitiveGetCurrentWorkingDirectoryAsBytes", (void*)primitiveGetCurrentWorkingDirectoryAsBytes},
	...
	{"UnixOSProcessPlugin", "primitiveRealpathAsBytes", (void*)primitiveRealpathAsBytes},
	...
	{"UnixOSProcessPlugin", "primitiveEnvironmentAtAsBytes", (void*)primitiveEnvironmentAtAsBytes},
	...
	{"UnixOSProcessPlugin", "primitivePathconf", (void*)primitivePathconf},
	{"UnixOSProcessPlugin", "primitiveSendSigintTo", (void*)primitiveSendSigintTo},
	{"UnixOSProcessPlugin", "primitiveSpaceForByteSizeInDirectoryPath", (void*)primitiveSpaceForByteSizeInDirectoryPath},
	...
	{"UnixOSProcessPlugin", "primitiveSysconf", (void*)primitiveSysconf},
	...
	{"UnixOSProcessPlugin", "primitiveFpathconf", (void*)primitiveFpathconf},
	...
	{"UnixOSProcessPlugin", "listOfCStringsInwithOffsets", (void*)listOfCStringsInwithOffsets},
	...
	{"UnixOSProcessPlugin", "primitiveEnvironmentAtSymbolAsBytes", (void*)primitiveEnvironmentAtSymbolAsBytes},
